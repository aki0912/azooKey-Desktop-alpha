...
def main() -> int:
    ...
    context_records = [json.loads(s) for s in (ROOT/'fixtures/context_pairs.jsonl').read_text(encoding='utf-8').splitlines() if s.strip()]
    validate_record_set(records + context_records)
    ...
    try:
        ...
        for schema_name, instances in [('span_record.schema.json', records + context_records),('language_model.schema.json',[model])]:
    ...
    print(f'Span records: {len(records)} valid; context contracts: {len(context_records)} valid; golden vectors: {len(vectors)}; event contracts: {len(events)} structurally valid.')
