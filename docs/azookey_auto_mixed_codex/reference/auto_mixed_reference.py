...
def validate_span_record(record: Mapping[str, Any]) -> None:
    ...
    if not isinstance(available, bool):
        raise ValueError("context_available must be boolean")
    if available != ("left_context" in record):
        raise ValueError("left_context must be present exactly when context_available is true")
    if available:
        context = record["left_context"]
        if not isinstance(context, str):
            raise ValueError("left_context must be string")
        check_scalar_text(context)
        if len(context) > 30:
            raise ValueError("left_context exceeds 30 scalars")
    cursor = 0
