...
class RecordTests(unittest.TestCase):
    ...
    def test_context_contrast_contracts(self):
        cases = [json.loads(line) for line in (ROOT/'fixtures/context_pairs.jsonl').read_text(encoding='utf-8').splitlines()]
        validate_record_set(self.records() + cases)
        paired = [case for case in cases if case['raw'] == 'made' and case['context_available']]
        self.assertEqual({case['spans'][0]['label'] for case in paired}, {'RAW', 'JA_ROMAN'})
        self.assertEqual({case['group_id'] for case in paired}, {'context-made-contrast'})

    def test_context_availability_must_be_explicit(self):
        case = json.loads((ROOT/'fixtures/context_pairs.jsonl').read_text(encoding='utf-8').splitlines()[0])
        del case['left_context']
        with self.assertRaises(ValueError):
            validate_span_record(case)
        case['left_context'] = 'I ' * 16
        with self.assertRaises(ValueError):
            validate_span_record(case)
