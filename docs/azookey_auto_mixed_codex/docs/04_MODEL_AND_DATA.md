...
- 英語条件に届かなければ、標準入力表で成立するローマ字を日本語として表示する。従来の漢字候補／かな末尾の境界を維持できる場合は維持する。ただし、完成した読みで末尾の各位置にも十分な日本語根拠がある場合は全体変換へ戻す（2026-09-25追補、03章§11.7）。追加で日本語扱いにする部分はpJA平均がモデルのhold値（現在0.65）以上なら既存変換候補、未満ならかなの読みを表示する。末尾の未完英字はrawのまま付ける。内部の未完子音や不成立入力は原文保持する。
