...
- URL、メール、コード、略語、Unicode literal、空白の保護を先に適用する。保護後の連続する推定対象を1区間とし、下記の辞書候補・スコア・独立した前後区間の条件を使って日英を分ける。
...
- **埋め込まれた英単語の保持（2026-09-25更新）**：保護後のinferred区間内で、最小長は上記prefixと同じ3文字、最大32文字の辞書完全一致を、日本語優先の全体表示より先に確認する。候補の端をViterbiのRAW runへ限定しない。探索候補数は区間長×最大単語長以内。英語の既存スコア条件に加え、残る左右の日本語が独立にローマ字として成立し、各側の平均pJAがhold値以上であることを要求する。本文末尾の未完子音だけの側は、表示が原文と同じなのでhold条件を要求せずそのまま残す。内部の未完子音は不可。適格候補のうち最長の1語、同長なら開始位置が先の語を区間ごとに採用する。区間全体が辞書にある単語なら、その全体判定を優先し、短い別単語へ分割しない。例えばmadeを英語として採らなかった後でmad＋eを作らない。空白・記号・保護token・書記素の途中は越えず、探索中にZenzaiを呼ばない。meeting固有の規則や閾値緩和は追加しない。
- **確定文脈による入力単位の分断を防ぐ（2026-09-25追補）**：上記候補が、読みとして成立する区間のローマ字入力単位を途中で区切る場合は、文脈なしの同じ原文でも候補の英語条件を満たすことを要求する。確定文脈がある場合だけの追加検査で、`hennkoutennga` の `nn` の途中から `hen` を切り出す誤判定を防ぐ。標準入力表の独立境界を使い、単語ごとの例外は設けない。判定手順とスコア共有は03章§11.9を参照。
...
- 英語条件に届かなければ、標準入力表で成立するローマ字を日本語として表示する。従来の漢字候補／かな末尾の境界を維持できる場合は維持する。ただし、完成した読みで末尾の各位置にも十分な日本語根拠がある場合は全体変換へ戻す（2026-09-25追補、03章§11.7）。確定文脈だけで全体がかな表示に落ちる場合は、辞書語・prefixを除いて文脈なしの既存runtime判定と対照する（03章§11.8）。追加で日本語扱いにする部分はpJA平均がモデルのhold値（現在0.65）以上なら既存変換候補、未満ならかなの読みを表示する。末尾の未完英字はrawのまま付ける。内部の未完子音や不成立入力は原文保持する。
- 上記の辞書候補と妥当な両側区間が成立しなければ、従来の日本語優先処理へ進む。全体がローマ字として不成立の場合は、既存判定器が提案した境界を使う従来経路も保持する。任意のsubstringにZenzaiを呼ぶ探索は行わない。複数の埋込み英単語を同時に最適化する探索は未実装。

2026-09-25の仕様差分は辞書候補の境界に限る。再学習候補で `meetingg` が一つのRAW runになると、既知のmeetingまで照合できなかったため変更した。候補が増えるので、日本語中の英単語に似た部分を誤抽出する影響をdevの逐次入力で比較する。モデル係数・特徴量・文脈・採用閾値、manual入力は変更しない。
...
**2026-09-25追補：入力途中の重みを保つ比較実験。** 任意の学習config `sample_weighting={"policy":"prefix-mass-v1","prefix_fraction":0.5}` を追加した。指定時だけ、各train原文の総重み1をprefix群と非prefix群へ指定比率で配り、それぞれの学習対象位置へ均等配分する。記号・文脈variantの増加でprefix群の重みが薄まらないことが目的。非prefix群には原文・ローマ字・記号・文脈variantが入り、意図的に未完な原文もこちらに属する。一方に学習対象位置がない原文は、残る群へ全重みを配る。どちらもない原文は学習対象外。比率0/1や未知の増強種別は拒否する。

省略時は全対象位置へ均等配分する従来式を維持する。dev・calibrationは従来どおり原文のみ、testは調整に使わない。分割、原文、prefixの生成個数・生成元、モデルschema、特徴量定義は変更しない。学習manifestには配分規則と実際の増強種別ごとの重みを保存する。0.5は比較用の配分であり、最適値・既定値・実用品質の保証ではない。記号variantの寄与が減る影響を含め、完成文と実runtimeの入力途中を合わせて比較する。文脈variantからのprefix追加は別の未実装項目として残る。
