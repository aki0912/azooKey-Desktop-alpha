...
作成日：2026-09-23 ／ 設計版：1.1（T2完了後の方針変更） ／ 仮称：Auto Mixed Input

**T2まで実装済みのフォーク向け**：まず `CODEX_CONTINUE_AFTER_T2.md` と `docs/09_T2_MIGRATION.md` を読む。既存のT2実装・golden・数値仕様v1は維持し、T3で文脈付き判定v2を追加して比較する。実際のフォークや `implementation_status.md` はこのZIPには含まれず、T2完了は利用者の申告である。
...
| `CODEX_CONTINUE_AFTER_T2.md` | T2完了済みフォークへの差分導入指示 |
| `docs/09_T2_MIGRATION.md` | 文脈付き判定v2、曖昧語の条件付き保留、移行・検証契約 |
...
3. `made`、`no`、`to`などは、利用可能な文脈を見て判定する。文脈が不足し確信がない場合に原文表示を選び、区間修正を可能にする。綴りだけを理由に常時保留しない。
...
区間意図データ50件、文脈対照例5件、キー操作等の契約16件、v1特徴量・数値golden128件、参照コード24テストを同梱。学習済み日英判定モデルやv2の推論実装は含まない。`language_model_fixture.json`はテスト専用の人工係数。
