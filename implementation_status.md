...
2026-09-25時点。**自動入力でEscapeのあとにTabまたはShift+Tabを押すと、原文表示を解除して通常の変換候補を開くようにした。** 最新記録は末尾の「Escape後のTabで通常変換を再開」を参照。各節の検証結果と未確認事項は、その作業時点の記録である。
...
## 「変更点が」のローマ字を英単語として分断する問題（2026-09-25）

`hennkoutennga` が `hen` と `nkoutennga` に分かれる問題を修正した。固定の日本語文脈を添えた再現試験で、「変更点が」の先頭表示・候補選択・削除・再入力・確定を確認した。作業開始時のHEADは `37c79a4`、working treeはclean。今回の修正はソースとテストまでで、インストール済みIMEにはまだ反映していない。

原因は、確定文脈が日本語スコアを過度に下げ、英単語辞書にある `hen` の切り出しを許すことだった。標準入力表では `nn` が一つの入力単位だが、その途中で区切ると後半だけが「んこうてんが」として変換される。調査時は「henンコウテンが」を再現した。利用者報告の漢字との完全一致は未確認だが、試験用に `hen` の判定を外すと「変更点が」が第1候補となり、誤分割を切り分けられた。

修正後は、確定文脈があるとき、読みとして成立する区間の入力単位を跨ぐ英単語候補に、文脈なしの原文スコアでも既存の英語条件を満たすことを要求する。境界は標準ComposingTextから取得する。`meeting` のような本来の英語は綴りの根拠があれば維持する。追加スコアは既存の単語全体判定と共有し、この検査による計算は1回のsegment呼出しにつき最大1回。モデル・閾値・辞書の変更や単語別の例外はない。仕様は設計書03章§11.9と04章§3.3に追記した。

検証ログは `build/auto-mixed/hennkou-fix-20260925/` に保存した。

- 修正前は新規3試験で48 assertion失敗（`before.log`）。最初の修正後は43試験中1項目だけ失敗した（`core.log`）。追加試験の `kitte` は `kitten` の辞書prefixでもあり、既存仕様ではかな表示を保つため、漢字変換区間を要求した期待値を訂正した。`kit` と `te` に分断しないことは引き続き検証する。
- 最終の関連回帰は **87試験・15 suite成功、skipなし、46.339秒**（`regression.log`）。通常辞書と実Zenzai、確定文脈なし／空／固定日本語2種、逐次入力と一括入力、Tab先頭候補、読み削除、再入力、確定、Escape原文回復、child解放を確認した。meeting・sample・apple、長音、句読点、不成立ローマ字、13階の詳細候補、transportも成功。人工モデルでは `nn`・促音の分断防止、Unicode範囲、独立した英語の根拠がある混在入力の維持を検証した。
- 特徴量・LR・Viterbi・Python参照・fixture／承認済みexport・原文表示の数値回帰は **28試験・7 suite成功、skipなし、0.168秒**（`parity.log`）。既存の参照成果物を使用し、再学習はしていない。モデルSHAは `471a88a65739d72386d57fef1531c0a3aa0a031f9c4a709a0fcb4eca728baa22` のまま。

macOS 27 arm64／Swift 6.4のRelease構成、変換学習OFFの隔離セッションで実施した。前回調査の制限付き実行ではZenzai初期化に失敗したため、実Zenzaiの試験は通常の実行環境で行った。SwiftPM native非推奨・既存の依存警告は残る。全Core試験、SwiftLint（未導入）、IMEのパッケージ更新・実Mach XPC・物理打鍵、長文の性能計測は今回未実施。ソースの修正と関連回帰は完了し、差分は未コミット。

### 修正版をMixedへ反映し、実サーバーで検証（2026-09-25）

利用者の「インストールして」に従い、修正版をMixed専用Releaseとしてビルド・更新した。更新済みサーバーへの実Mach XPC試験は **13件成功、skipなし、27.860秒**。`hennkoutennga` の新規試験も成功し、「変更点が」の表示と確定を確認した。ログと旧版の退避先は `build/auto-mixed/hennkou-install-20260925/`。

ビルド後、署名・識別子を `update --dry-run` で検証してから専用updateを実行した。インストール先とビルド成果物のIME/helper、モデル、アイコン、GGUF・marisa、構成ファイルのSHAは一致。モデルと資源は更新前と同一で、Release・診断OFFを維持した。IME SHAは `8aa50d1dd8446f607bfaf2c7a75579c47b0f49bcef7965bc160e5fd57208c8de`、helper SHAは `f6efcbc3b1a3a661751359f1710818e82728b5b088f08d86b895abda2e73ff7b`。

制限付き環境のcodesignは信頼検証エラーになったが、通常環境でのdeep strict検証は成功した（`signature-verified.log`）。更新前後の選択ソースはmacOS標準日本語で一致し、Mixedの登録・有効状態も同一だった。通常版への操作や入力ソースの切替は行っていない。

追加の実XPC試験では、左文脈なし／空／固定日本語2種で逐次入力、一括入力、Tab先頭候補、候補採用、読み削除、再入力、古いrevisionの拒否、確定・ack、Escape原文回復を確認した。既存の英日混在、長音、句読点、開発、13階の候補操作も成功。1,000回の入力・確定・ack試験を含む（`installed.log`）。実アプリでの物理打鍵と長文の性能分布は未確認。インストールと実サーバー検証まで完了し、コード・検証記録の差分は未コミット。

## Option・Control両対応の文字種変換（2026-09-25）

開始HEADは `b769d01`、working treeはclean。利用者の指定に従い、通常入力と自動入力に未確定部分全体の文字種変換を実装した。割り当てと操作方法はREADMEの「入力中の文字種変換」に記載。候補選択中も全体をプレビューし、Enterで表示どおりに一度だけ確定する。Control + ;は半角英字へ変更し、Control + 'は別名として残した。

原文を保持して文字種を相互に切り替え、追加入力・削除でも指定を維持する。Escape、通常入力のSpace、自動入力のTabで指定を解除し、確定・空バッファ・入力終了時にもリセットする。自動入力のSpaceは空白挿入を維持。共通のキー判定、既存入力表による読み生成、表示範囲、応答待ちでフォーカスが終了した場合の表示確定を実装した。純粋な状態機械と入力表への依存を分け、既存の軽量テストも維持した。

検証ログと旧版の退避先は `build/auto-mixed/character-type-20260925/`。

- Core標準実行は **251件中188件成功、63件skip**（`full.log`）。skipは外部モデル・実Zenzai・導入済みIMEなどを明示的に有効化する試験。これらをすべて成功と扱わない。
- 依存ライブラリを使わない軽量Core試験は **50件中46件成功、4件skip**（`pure.log`）。Core標準実行と重複する。途中で見つかった新共通型への依存不足は、読み生成を変換器側へ分離して解消した。
- 通常入力とMixedのクライアント試験は **14件成功**（`client.log`）、Pythonのツール試験は **16件成功**（`tools.log`）。SwiftLint strictと差分空白検査も成功。
- `main → マイn → main`、大文字・未完子音・記号・絵文字、全割り当て、JISのコロンとUSのShift併用、候補選択からの全体変換、編集と解除、ライブ変換ON／OFF、確定後のリセットを検証。Mixedでは応答待ち・古い応答・フォーカス終了時の重複確定防止も確認した。
- 最終版のmacOS Releaseビルド・署名検証・Mixed専用updateが成功（`build.log`／`update-check.log`／`update.log`）。ビルド先と導入先のIME/helper・モデル・資源など22ファイルのSHAが一致した（`installed-hashes.json`）。Release・診断OFF。通常版の導入先は変更していない。
- 更新済みhelperに対する **実Mach XPCの追加試験1件成功、skipなし**（`installed.log`）。通常入力と自動入力のそれぞれでOption・Controlの12通りを送り、文字種切替時に確定しないこと、原文どおりの追加入力・削除、Enterでの確定値と効果が1件だけになることを確認した。

TextEditでのUIツールによる入力も試みたが、Mixed自動を選択しても試験文字が直接入力となり、IME経由の打鍵確認には使えなかった。物理キーボードによる実アプリの確認は未実施。試験用文書を閉じ、入力ソースを開始時のABCへ戻した。macOS 27 arm64／Xcode 27／Swift 6.4で検証し、依存のdeployment targetなど既存のビルド警告は残る。差分は未コミット。

## Escapeを2回押したときの全削除（2026-09-25）

開始HEADは `476bad3`、working treeはclean。利用者の指定に従い、自動入力でEscapeを続けて2回押すと未確定入力をすべて削除するよう変更した。1回目は従来どおり文字種指定を解除、候補を閉じる、または原文表示へ戻す。入力・削除・候補操作・文字種切替を挟むと回数をリセットする。通常入力の文字種指定中は、従来から1回目で指定解除、2回目で全削除となるため、その回帰試験を追加した。

応答待ちのキーを保持するクライアントにもEscapeの状態を記録し、2回目の応答前にフォーカスが終了したり通信が失敗したりしても、削除した入力を復元・確定しないようにした。古い応答の拒否、空バッファと次の入力への状態リセットも検証した。既存の「2回目でも原文を残す」試験は新仕様へ更新し、1回目の原文表示と候補生成の回帰を維持した。

検証ログと旧版の退避先は `build/auto-mixed/escape-twice-20260925/`。Core標準実行は **254件中191件成功、63件skip**（`full.log`）。軽量Coreは **52件中48件成功、4件skip**（`pure.log`、標準実行と重複）、クライアントは **15件成功**（`client.log`）。skipは明示有効化が必要な外部モデル等の試験。SwiftLint strict、差分空白検査、macOS Releaseビルド、署名・識別子検証も成功した。

Mixed専用updateで反映し、ビルド先と導入先のIME/helper・モデル・資源など22ファイルのSHA一致を確認した。更新済みhelperへの **実Mach XPC試験1件成功、skipなし**（`installed.log`）。通常入力と自動入力で文字種変換後にEscapeを2回送り、1回目で入力保持、2回目で表示と原文が空になり、新しい確定効果が発生しないことを確認した。通常版の導入先と入力ソース選択は変更していない。物理キーボードでの確認は今回未実施。差分は未コミット。

## Escape後のTabで通常変換を再開（2026-09-25）

開始HEADは `9dbdba6`、working treeはclean。利用者の指定に従い、自動入力でEscapeにより原文表示へ戻したあと、TabまたはShift+Tabで通常の変換を再開して候補一覧を開くようにした。原因はTabの処理で原文表示の状態が残り、原文だけの候補を開いていたこと。文字種指定・読み表示からの復帰と同じ処理で原文を再評価し、候補選択に進めるよう変更した。

原文を保持したまま日本語候補と通常の記号表記へ戻り、英語・絵文字の保持、候補採用と確定、詳細候補一覧での再計算抑制、Escapeを続けて2回押したときの全削除を確認した。従来の「Escape後のTabでも原文候補を維持する」期待値は新仕様に更新した。通常入力のキー割り当てとXPC形式は変更していない。

ログと旧版の退避先は `build/auto-mixed/escape-tab-20260925/`。Core標準実行は **255件中192件成功、63件skip**（`full.log`）、軽量Coreは **53件中49件成功、4件skip**（`pure.log`、標準実行と重複）、クライアントは **15件成功**（`client.log`）。skipは外部モデル等の明示有効化が必要な試験。SwiftLint strictと差分空白検査も成功した。

macOS Releaseビルドと署名検証後、Mixed専用updateで反映した。最初の更新はMixed選択中の安全チェックで止まり、導入済みファイルは変更されなかった。その後、一時的にABCへ切り替えて更新し、開始時のMixed自動モードへ戻したことを確認した（`source-before.txt`／`source-after.txt`）。ビルド先と導入先のIME/helper・モデル・資源など22ファイルのSHAが一致した。

更新済みhelperへの **実Mach XPC試験2件成功、skipなし**（`installed.log`）。`13kai → Escape → Tab` で「回」「階」の通常候補が出て「13階」を採用・確定できること、文字種変換とEscape2回の全削除が引き続き動くことを確認した。通常版の導入先は変更していない。物理キーボードでの確認は今回未実施。差分は未コミット。
