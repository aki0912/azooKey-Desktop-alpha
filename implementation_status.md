...
2026-09-25時点。**通常入力と自動入力の両方に、Option・Control・F6〜F10による未確定部分全体の文字種変換を追加した。`main → マイn → main` の往復とEnterでの確定に対応する。** 最新記録は末尾の「Option・Control両対応の文字種変換」を参照。各節の検証結果と未確認事項は、その作業時点の記録である。
