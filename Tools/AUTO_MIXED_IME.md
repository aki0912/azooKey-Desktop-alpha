...
- `- . , [ ] ? !` は日本語優先で `ー 。 、 「 」 ？ ！` を表示・確定する。英語の直後は半角を保つ。例：`asita?` → `明日？`、`asita!` → `明日！`、`apple?!` → `apple?!`、`[apple]` → `「apple」`。空白の後は日本語優先に戻り、閉じ括弧は開き括弧に合わせる。
...
- URL・ファイル名などの保護tokenと、数字に接する記号は半角を保つ。確定直後の判断には取得可能な短い左文脈を使い、取得できなければ日本語優先。Escapeで元のASCII記号へ戻せる。対象は上記7記号で、ほかの記号は従来どおり。
