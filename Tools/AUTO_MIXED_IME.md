...
- Escapeは1回目で文字種指定の解除・候補を閉じる・原文表示へ戻す操作を行う。続けて2回目を押すと未確定入力をすべて削除する。途中に入力・削除・候補操作を挟むと回数をリセットする。確定済みの文章は削除しない。
- [文字種変換のショートカット](../README.md#入力中の文字種変換)は通常入力と共通。Option + A／S、Control + ;／:、F10で未確定部分全体を元の半角英字へ戻せる。文字種の指定は追加入力・削除でも維持し、Enterで確定する。EscapeまたはTabで指定を解除する。
- 文字とSpaceは原文へ追加する。Spaceは空白を入力する。
- Tab/Shift-Tabは候補移動。Escapeで原文表示へ戻したあとも、Tab/Shift-Tabで通常の変換を再開して候補一覧を開く。日本語の候補一覧を開くときだけ詳細な候補を生成し、一覧内の移動では再計算しない。入力中は従来の候補生成を維持する。対象区間を強調し、Enterで候補採用、候補選択外のEnterで全体確定。
...
- 英単語に続く日本語も長音込みで評価する。`samplenode-ta` → `sampleのデータ`。判定が弱い入力途中の後続部分は原文で保留し、辞書登録語である英語部分の境界を保つ。`sample-data` のようにハイフンの両側が英単語なら英語表記を維持する。
