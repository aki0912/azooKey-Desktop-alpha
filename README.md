...
Escapeで文字種指定を解除し、続けてもう一度Escapeを押すと未確定入力をすべて削除します。自動入力では、候補を閉じたあとや原文表示へ戻したあとも、続けてEscapeを押すと全削除します。途中に入力・削除・候補操作を挟むとEscapeの回数はリセットされます。通常入力のSpace、自動入力のTabで候補操作へ戻る場合も文字種指定を解除します。自動入力のSpaceは空白を追加します。未確定文字がない場合は文字種変換を行いません。Control + ;は従来の半角カタカナから半角英字へ変更しました。半角カタカナにはF8を使ってください。
