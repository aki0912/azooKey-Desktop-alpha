...
Escapeで文字種指定を解除できます。通常入力のSpace、自動入力のTabで候補操作へ戻る場合も指定を解除します。自動入力のSpaceは空白を追加します。未確定文字がない場合は文字種変換を行いません。Control + ;は従来の半角カタカナから半角英字へ変更しました。半角カタカナにはF8を使ってください。
