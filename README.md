...
通常入力・Mixedの自動入力とも、次のキーで未確定部分全体の文字種を変更できます。

| 変換先 | Option系 | Control系 | ファンクションキー |
|---|---|---|---|
| ひらがな | Option + Z | Control + J | F6 |
| カタカナ | Option + X | Control + K | F7 |
| 半角英字 | Option + A／S | Control + ;／: | F10 |
| 全角英字 | Option + C | Control + L | F9 |
| 半角カタカナ | — | — | F8 |

たとえば、`main` と入力して「マイn」と表示された場合、Option + Aで元の `main` に戻せます。Control + 'も半角英字に使えます。US配列でControl + :を入力するときはShiftを併用します。ファンクションキーはキーボードの設定によってFnの併用が必要です。

文字種を変えてもすぐには確定しません。別の文字種へ切り替えたり、文字を追加・削除したりしてから、Enterで表示どおりに確定します。確定すると文字種の指定は解除され、次の入力は通常の変換・自動判定に戻ります。

Escapeで文字種指定を解除し、続けてもう一度Escapeを押すと未確定入力をすべて削除します。自動入力では、候補を閉じたあとや原文表示へ戻したあとも、続けてEscapeを押すと全削除します。途中に入力・削除・候補操作を挟むとEscapeの回数はリセットされます。通常入力のSpace、自動入力のTabで候補操作へ戻る場合も文字種指定を解除します。自動入力でEscapeにより原文表示へ戻したあとも、TabまたはShift+Tabで通常の変換候補を開けます。自動入力のSpaceは空白を追加します。未確定文字がない場合は文字種変換を行いません。Control + ;は従来の半角カタカナから半角英字へ変更しました。半角カタカナにはF8を使ってください。
